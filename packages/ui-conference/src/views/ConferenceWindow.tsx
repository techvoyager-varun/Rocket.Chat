...
const ConferenceWindow = () => {
	const { room, session, call, actions, slots, thread } = useConference();
	...
	const otherMembers = call.canRing && session.joined ? call.members.filter((m) => m._id !== user?._id && !isInVideoConference(m)) : [];
