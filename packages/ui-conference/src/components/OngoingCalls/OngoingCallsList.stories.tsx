 ...
 * user's sessions gets no Silence button, because there is nothing here to silence. That comes from the
 * video-conf context, which is why those stories build a different app root.
 */
const withCalls = (value: Partial<OngoingCallsContextValue>, incoming: { callId: string; dismissed: boolean }[] = []) => [
	withLiveOngoingCalls(value),
	withCallProviders(conferenceAppRoot().withIncomingCalls(incoming as any)),
];
...
export const Ringing: Story = {
	decorators: withCalls({ ringing: [ringing] }, [{ callId: 'ringing', dismissed: false }]),
};
...
export const RingingSilenced: Story = {
	decorators: withCalls({ ringing: [ringing], silencedCalls: ['ringing'] }, [{ callId: 'ringing', dismissed: false }]),
};
