...
const renderRow = (call = standup, { silencedCalls = [] as string[], audible = true } = {}) => {
	const AppRoot = mockAppRoot()
		.withJohnDoe()
		.withIncomingCalls(audible ? ([{ callId: call.callId, dismissed: false }] as any) : ([] as any))
		.build();

	const calls = buildOngoingCallsContext({ joinCall, declineCall, silenceCall, silencedCalls });
